...
function rewrite() {
    ...
    $(data).each(function (index, val) {
        if (state === "wait") {
            if (val.isDone === 0) {
                str += "<li id=" + index + "><span class='con'>" + val.con + "</span><span>" + val.hms + "</span><span class='date'>" + val.time + "</span><span><i class='iconfont'>&#xe6dc;</i></span><div class='pos'><div class='dosuc'>完成</div></div></li>";
            }
        } else if (state == "done") {
            if (val.isDone === 1) {
                str += "<li id=" + index + "><span class='con'>" + val.con + "</span><span>" + val.hms + "</span><span class='date'>" + val.time + "</span><div class='pos'><div class='del'>删除</div></div></li>";
            }
        }
    });
    $(".content").html(str);
...
//完成事件
$(".bottom").on("click",".dosuc",function(){
    var data = getData();
